...
type ProviderStatus = { configured: boolean; provider: string; baseUrlHostname: string; baseUrlHost?: string; model: string; configSource?: string; keyConfigured?: boolean; blockers?: string[]; status: string; lastTestedAt: string | null; sanitizedError: string | null };
  ...
  return <main className="app-shell app-studio-shell" data-theme={theme}>
    ...
    <div className="workspace-grid app-dashboard-grid">
      <aside className="sidebar app-sidebar">
        ...
        {providerStatus&&<div className="provider-box compact-provider"><strong>{providerStatus.configured?"Configured":"Not configured"}</strong><small>{providerStatus.provider} - {providerStatus.model||"no model"}</small><small>Source: {providerStatus.configSource || "disabled"}</small><small>Host: {providerStatus.baseUrlHost || providerStatus.baseUrlHostname || "invalid-url"}</small><span className={`risk ${providerStatus.status==="ok"?"low":providerStatus.status==="error"?"critical":"medium"}`}>{providerStatus.status}</span>{!providerStatus.configured&&providerStatus.blockers?.length?<p className="notice">{providerStatus.blockers.join(" · ")}</p>:null}{providerStatus.sanitizedError&&<p className="error">{providerStatus.sanitizedError}</p>}<button className="secondary" onClick={()=>void testProvider()}>Test connection</button></div>}
