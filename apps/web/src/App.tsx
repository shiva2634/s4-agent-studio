...
function MediaStudio({path,navigate}:{path:string;navigate:(path:string)=>void}) {
  ...
  async function generateAudioVideo(sceneId: string) {
    ...
    const response = await fetch(`${API}/api/media/projects/${selectedId}/scenes/${sceneId}/generate`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ task: "AUDIO_VIDEO", approved: true, paidProviderApproved: true, maxAttempts: 3 })
    });
    const data = await response.json();
...
function AssetItem({projectId,sceneId,asset,onReplaceAsset,onDeleteAsset,onUpdateAudio,onSelectBackgroundMusic,onApproveAsset,onRejectAsset}:{projectId:string;sceneId:string;asset:MediaAsset;onReplaceAsset:(sceneId:string,assetId:string,file:File|undefined)=>Promise<void>;onDeleteAsset:(sceneId:string,assetId:string)=>Promise<void>;onUpdateAudio:(assetId:string,settings:Partial<AudioSettings>)=>Promise<void>;onSelectBackgroundMusic:(assetId:string|null)=>Promise<void>;onApproveAsset:(assetId:string)=>Promise<void>;onRejectAsset:(assetId:string,feedback:string)=>Promise<void>}) {
